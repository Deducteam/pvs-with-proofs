...
;; cons of substit and a certificate of ctx(obj[alist]) wit a hole of ctx(subst obj list)
(defun substit-with-proofs (obj alist ctx)
  ;; At some point, should verify that car of every element of the
  ;; alist is a declaration.
  (cond ((or (null obj) (null alist))
	 (cons obj (make-trace 'proof-hole)))
	(t #+pvsdebug
	   ...
	   (let* ((fvars (freevars obj))
		 ...
		 (new-substit-hash
		  (let* ((sobj (substit* obj nalist))
			 (subobj (compute-substit obj nalist)))
		    (if (tc-eq sobj subobj)
			(cons sobj (make-trace 'proof-hole))
		      (cons sobj (make-trace 'deep-trusted-rule
					     (list ctx 'substit sobj subobj)
	   ...
	   (let* ((fvars (freevars obj))
		 ...
		 (new-substit-hash
		  (let ((sobj (substit* obj nalist)))
		    sobj)))))))
;; (defun substit (obj alist)
;;   (car (substit-with-proofs obj alist (make-ctx 'not-implemented 'c128))))
      ...
      (cond ((and (not *substit-dont-simplify*)
	    ...
	    (t
	     (let* ((stype (find-supertype (type op)))
		    (nex
		     (if *compute-substit*
			 (typecheck (compute-application op arg)
				    :expected
...
(defmethod substit* ((expr binding-expr) alist)
  (if (not (substit-possible? expr alist))
      ...
      (let* ((*leave-bindings-undeclared* (memq expr *let-operators*))
	     ...
	     (new-bindings (if (or *compute-instantiate*
				   (equal new-bindings-i (bindings expr)))
			       (bindings expr)
			       new-bindings-i))
