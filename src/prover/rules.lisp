...
(defun prop-axiom-rule ()
  #'(lambda (ps)
	...
	(if (eq (car (check-prop-axiom s-forms)) '!)
	    (values '! nil)
	    (values 'X nil)))))
...
(defmethod negate-with-proofs ((formula negation))
  (cons (argument formula)
	(cons (make-trace 'not-not-rule
		    (list (argument formula))
		    (list (make-trace 'proof-hole)))
	      (make-trace 'cut-rule
			  (list formula)
			  (list (make-trace 'axiom-rule (list (argument formula)))
...
(defmethod negate-with-proofs (formula)
  (make-negation-with-proofs formula))
...
(defun check-prop-axiom* (forms)
  (if (consp forms)
      (let ((result (essentially-true? (car forms) (cdr forms))))
	(if (car result)
	    (cons '! (cdr result))
	  (check-prop-axiom* (cdr forms))))
    (cons 'X (make-trace 'empty 'check-prop-axiom*))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;INSTRUMENTED
(defmethod essentially-true? ((form name-expr) remforms)
  (if (tc-eq form *true*)
      (cons t (make-trace 'true-rule nil nil))
    (if (member form remforms :test #'neg-tc-eq)
        (cons t (make-trace 'not-implemented 'p12))
      (cons nil (make-trace 'empty 'essentially-true?)))))

(defmethod essentially-true? ((form negation) remforms)
  (with-slots (argument) form
      (if (tc-eq argument *false*)
	  (cons t (make-trace 'not-false-rule nil nil))
	(if (member argument remforms :test #'tc-eq)
	    (cons t (make-trace 'axiom-rule (list argument)))
	  (cons nil (make-trace 'empty 'essentially-true?))))))

(defmethod essentially-true? ((form equation) remforms)
  (with-slots (argument) form
       (if (apply #'tc-eq (exprs argument))
	   (cons t (make-trace 'refl-rule (list (car (exprs argument)))))
	 (if (member form remforms :test #'neg-tc-eq)
	     (cons t (make-trace 'axiom-rule (list form)))
	   (cons nil (make-trace 'empty 'essentially-true?))))))

(defmethod essentially-true? ((form expr) remforms)
  (if (member form remforms :test #'neg-tc-eq)
      (cons t (make-trace 'axiom-rule (list form) nil))
    (cons nil (make-trace 'empty 'essentially-true?))))
...
(defun delete-rule-fun (sformnums)
  #'(lambda (ps)
	...
	(multiple-value-bind
	    (signal subgoal)
	    (sequent-reduce-with-proofs goalsequent
			    #'(lambda (sform)
				(values (cons '?
					      (make-trace 'weak-rule
							  (list (formula sform))
	...
	(if (or (typep strat 'strategy)
	    ...
	    (let* ((new-strat (if (typep strat 'strategy)
		  ...
		  (progn (clear-strategy-errors)
			 ...
			 (values (cons 'X
				       (make-trace 'empty 'np15))
				 nil
				 nil))
		(let* ((trace (collect-traces newps))
		       (dummy (check-traces-recursively newps "WRONG APPLY-STEP"))
		       (subgoals (collect-subgoals newps))
		  ...
		  (check-trace trace
			       (mapcar #'(lambda (subgoal)
					   (if (consp subgoal)
					       (car subgoal)
					     subgoal)) subgoals)
			       (current-goal ps) "APPLY BUG")
		  (if (eq (status-flag newps) 'XX)
		      (values (cons 'X
				    (make-trace 'empty 'np16))
			      nil
			      nil)
			...
			(if save?
			    (values (cons '?
					  (make-trace 'not-implemented 'p7))
				    (list (current-goal newps))
				    nil)
			  (values (cons 'X
					(make-trace 'empty 'np17))
				  nil
				  nil))
		      (if (eq (status-flag newps) '!) ;;(null subgoals)
			  (values (cons '! (cdr (status newps)))
				  nil
			...
			(values (cons '? trace)
				...
				(list 'dependent-decls *dependent-decls*
				      'current-xrule xrule))))))))
	  (values (cons 'X
			(make-trace 'empty 'np14))
		  nil
		  nil)))
    (cons (error-format-if "~%Error: timeout must be a positive integer")
	  (make-trace 'not-implemented 'p41))))

(defmethod collect-traces ((ps proofstate))
  (cond ((and (eq (status-flag ps) '!)
  	      (null (current-subgoal ps))
  	      (null (pending-subgoals ps))
  	      (null (remaining-subgoals ps)))
  	 (cdr (status ps)))
  	((and (eq (status-flag ps) '*)
  	      (or (null (current-subgoal ps))
		  (member (current-subgoal ps) (done-subgoals ps)));;FG ints/div/div_value 
  	      (null (remaining-subgoals ps)))
  	 (cond ((null (pending-subgoals ps))
  		(make-trace 'proof-hole))
  	       (t
  		(let* ((sorted-subgoals
			(sort (append (pending-subgoals ps) (done-subgoals ps) nil)
			      #'mystring<= :key #'label))
		       (sorted-traces (mapcar #'collect-traces sorted-subgoals)))
		  (catch-debug 'apply-trace
			       (apply-trace (cdr (status ps)) sorted-traces)
			       "COLLECT TRACES"
			       nil)))))
  	(t
  	 (make-trace 'not-implemented 'p17))))
  ...
  #'(lambda (ps)
	...
	(multiple-value-bind (sig subgoal)
	    (sequent-reduce
	     goalsequent
	     #'(lambda (sform)
		 (let ((result (call-process (car (negate-with-proofs (formula sform)))
					     *dp-state*)))
		   (cond ((false-p result)
			  (values '! nil))
...
(addrule 'copy (fnum) ()
  #'(lambda (ps)
	...
	(if sforms
	    (values (cons '? (make-trace 'contr-rule
					 (list (formula (car sforms)))
					 (list (make-trace 'proof-hole))))
