  ...
  (make!-conjunction*
   (mapcar #'(lambda (subgoal)
	       (let ((antes (mapcar #'(lambda (x)
					(car (negate-with-proofs (formula x))))
			      (neg-s-forms subgoal)))
...
(defun prove*-int (proofstate)
  (check-traces-recursively proofstate "WRONG PROOFSTEPPER INPUT")
  ...
  (let ((nextstate (proofstepper proofstate)))
    (check-traces-recursively nextstate "WRONG PROOFSTEPPER OUTPUT")
    (cond ((null (parent-proofstate proofstate))
	   (cond ((eq (status-flag proofstate) '!)
		 ...
		 ((and (or (fresh? proofstate)
		  ...
		  (setf (status proofstate)
			(cons '* (cdr (status proofstate))))
		  proofstate)
  ...
  (cond
   ((fresh? proofstate)   ;;new state
    (let ((post-proofstate ;;check if current goal is prop-axiom.
	   (let ((result (check-prop-axiom (s-forms (current-goal proofstate)))))
	     (cond ((eq (car result) '!) ;;set flag to proved! and update fields.
		    (check-trace (cdr result) nil (current-goal proofstate) "ILL TYPED PROOF 1")
		    (setf (status proofstate) result
		   ...
		   (t (catch-restore ;;in case of restore/enable interrupts
			 ...
			 (let* ((*rule-args-alist* nil)
			   ...
			   (let ((newps (rule-apply strategy proofstate)))
			     (check-traces-recursively newps "WRONG RULE-APPLY OUTPUT 1")
			     newps)))))))))
      ...
      (cond ((null post-proofstate) ;;hence aborted
	     (let ((nps	            ;;NSH(7.18.94) for proper restore
		    (nonstrat-parent-proofstate
		     proofstate)))
	       (setf (status nps) (cons nil (make-trace 'empty 'np1))
...
(defun success-step (proofstate)
  ...
  (let ((newtrace (catch-debug 'apply-trace
			       (apply-trace (cdr (status proofstate))
					    (mapcar #'(lambda (ps) (cdr (status ps)))
						    (done-subgoals proofstate)))
			       "SUCCESS STEP" nil)))
    (check-trace newtrace nil (current-goal proofstate) "ILL TYPED PROOF 2")
    (setf (status proofstate) (cons '! newtrace)))
  (mapcar #'(lambda (x)
	      (mapcar #'(lambda (y)
			  (pushnew y
				   (dependent-decls
...
;		   (label proofstate)))
	 (setf (status proofstate)
	       (cons '* (cdr (status proofstate))));;move
	 (next-proofstate proofstate))
	(t  (setf 
	     (remaining-subgoals proofstate)
...
(defmethod set-post-strat ((ps proofstate) stratfun)
  (cond ((null (remaining-subgoals ps))
	 (setf (status ps) (cons nil (make-trace 'empty 'np3))
	       (strategy ps)
	       (funcall stratfun ps)))
	(t   (setf (status ps)
		   (cons '? (make-trace 'not-implemented 'p5))
		   (strategy ps) nil)
...
(defmethod next-proofstate ((ps proofstate))
  (let ((par-ps (parent-proofstate ps))
    ...
    (cond ((null par-ps)    ;;hence root proof state
	   (if  (eq sflag '*)
		(setf (status ps) (cons 0 (cdr (status ps)))
		      (strategy ps)
	  ...
	  (t (cond ((memq sflag '(X XX))   ;;record failure or try something else.
		    (if (or (null (failure-strategy (strategy par-ps)))
			    (eq sflag 'XX))
			(setf (status par-ps) (cons 'X (make-trace 'empty 'np5)))
			(setf (status par-ps) (cons 0 (make-trace 'empty 'np6))
			      (strategy par-ps)
		   ...
		   ((eq sflag '*)
		    (cond ((and (typep ps 'strat-proofstate)
			   ...
			   (setf (status par-ps) (cons 0 (make-trace 'empty 'np7))
				 (strategy par-ps)
				 (failure-strategy (strategy par-ps))
				 (current-subgoal par-ps) nil))
...
(defun make-update (field value ps)
  ;;(break)
  (case field
	(status-flag (setf (status ps)
			   (cons value (make-trace 'not-implemented 'p6))))
...
(defun assert-tccforms* (tccforms ps)
  (if (null tccforms) nil
	  ...
	  (let ((*sequent-typealist* nil))
	    (assert-if-i (tccinfo-formula (car tccforms)) (make-ctx 'not-implemented 'c125)))
	(cond ((tc-eq value *true*)
	       (assert-tccforms* (cdr tccforms) ps))
	      ((eq (car sig) 'X) (cons (car tccforms)
				 (assert-tccforms*
				  (cdr tccforms) ps)))
...
(defun create-trace-if-necessary (ps name signal subgoals)
  (if (and (consp signal) (trace? (cdr signal)) (not (not-implemented-trace? (cdr signal))))
      signal
    (let ((signal (if (and (consp signal) (trace? (cdr signal))) (car signal) signal)))
    (if (not (or (eq signal '!) (eq signal '?)))
	(cons signal (make-trace 'empty name))
      (let* ((conclusion (append (mapcar #'formula (s-forms (current-goal ps)))
				 (mapcar #'formula (hidden-s-forms (current-goal ps)))
				 nil))
	     (hypotheses
	      (mapcar #'(lambda (subgoal)
			  (append (mapcar #'formula (s-forms subgoal))
				  (mapcar #'formula (hidden-s-forms subgoal))
				  nil))
		      (mapcar #'(lambda (subgoal) (if (consp subgoal) (car subgoal) subgoal))
			      subgoals)))
	     ;; (conc-hypos
	     ;;  (reduce
	     ;;   #'(lambda (conc-hypos form)
	     ;; 	   (let* ((conc (car conc-hypos))
	     ;; 		  (hypos (cadr conc-hypos)))
	     ;; 	     (if (every #'(lambda (hypo) (find form hypo :test #'compute-form-eq))
	     ;; 			hypos)
	     ;; 		 (list conc
	     ;; 		       (mapcar
	     ;; 			#'(lambda (hypo)
	     ;; 			    (remove form hypo :test #'compute-form-eq :count 1))
	     ;; 			hypos))
	     ;; 	       (list (cons form conc) hypos))))
	     ;;   (cons (list nil hypotheses) conclusion)))
	     ;; (conc (car conc-hypos))
	     ;; (hypos (cadr conc-hypos))
	     )
	(cons signal
	      ;; (progn (when (string= name "replace")
	      ;; 	(assert nil))
	      (make-trace 'trusted-rule
			  ;; (list name hypos conc)
			  (list name hypotheses conclusion)
			  (mapcar #'(lambda (subgoal)
				      (declare (ignore subgoal))
				      (make-trace 'proof-hole)) subgoals))))))))

(defun rule-apply (step ps)
  (check-traces-recursively ps "WRONG RULE-APPLY INPUT")
  (let* ((*ps* ps)
	 (* '*)
	 (*goal* (current-goal ps))
	 (*label* (label  ps))
    ...
    (cond ((typep step 'rule-instance);;if step is a rule, then
	   ...
	   (let ((newps (rule-apply (make-instance 'strategy
						   :topstep step)
				    ps)));;else step is a strategy
	     (check-traces-recursively newps "WRONG RULE-APPLY OUTPUT 2")
	     newps))
	  ((typep (topstep step) 'rule-instance)
	   (let* ((*tccforms* nil)
		 ...
		 ;; (handler-case
		 ;;  (catch-debug 'apply-trace (funcall (rule topstep) ps) ;; (break "rule-ap")
		 ;; 	   (format nil "problem in rule ~a" (rule-input topstep)) nil)
		 ;;  (condition (x)
		 ;; 	     (declare (ignore x))
		 ;; 	     (format t "problem in rule ~a" (rule-input topstep))
		 ;; 	     (assert (and 'catch nil))))
		 (catch-debug 'apply-trace (funcall (rule topstep) ps) ;; (break "rule-ap")
		 	   (format nil "problem in rule ~a" (rule-input topstep)) nil)		 
	      (let* ((signal (create-trace-if-necessary ps name signal* subgoals)))
                     (cond ((eq (car signal) '!);;success
			    (when (memq name *ruletrace*)
			      (decf *ruletracedepth*)
				...
				(when timeout-sect
				  (setf (rule-input topstep)
					(append (ldiff (rule-input topstep)
						       timeout-sect)
						(cddr timeout-sect))))))
			    (check-trace (cdr signal) nil (current-goal ps) "ILL TYPED PROOF 3")
			    (setf (status ps)
				  (cons '! (cdr signal))      
			    ...
			    ps)
			   ((eq (car signal) '?);;subgoals generated
			    (make-updates updates ps)
			    ...
			    (let* ((*tccforms* (remove-duplicates *tccforms*
			      ...
			      (cond (false-tccforms
				     ...
				     (setf (status ps)
					   (cons nil (make-trace 'empty 'np8));;start afresh
					   (strategy ps)
					...
					;updates;must be attached to subgoals.
					      ...
					      (subgoal-proofstates
					       (mapcar #'car subgoal-traces-proofstates))
					      (trace
					       (catch-debug
						'apply-trace
						(if (null tcc-subgoals)
						    (apply-trace
						     (cdr signal)
						     (mapcar #'cdr subgoal-traces-proofstates))
						  (apply-trace
						   (make-trace
						    'tcc-rule
						    (list
						     (mapcar
						      #'(lambda (subgoal)
							  (mapcar #'formula
								  (append (s-forms subgoal)
									  (hidden-s-forms subgoal)
									  nil)))
						      tcc-subgoals)
						     (mapcar #'formula
							     (append
			   ...
			   ((eq (car signal) 'X)
			    ...
			    (setf (status ps) (cons nil (make-trace 'empty 'np9));;start afresh
				  (strategy ps)
				  (failure-strategy step))
			    (make-updates updates ps)
			    ps)
			   ((eq (car signal) 'XX);;marks the current goal a failure
			    (setf (status ps) (cons 'XX (make-trace 'empty 'np10)))
			    ps)
			   ((eq (car signal) '*)
			    (setf (status ps)
				  (cons '* (make-trace 'not-implemented 'p9)))
			    ps)
			   (t  (undo-proof (car signal) ps)))))))
	  ((typep (topstep step) 'strategy)
	   (let ((subgoal-traces-proofstates
		  (make-subgoal-proofstates ps
					    (topstep step)
					    (list (current-goal ps)))))
	     (assert (and (consp subgoal-traces-proofstates)
			  (null (cdr subgoal-traces-proofstates))))
	     (check-trace (cdr (car subgoal-traces-proofstates))
			  (mapcar #'current-goal (mapcar #'car subgoal-traces-proofstates))
			  (current-goal ps)
			  "ILL TYPED PROOF 5")
	     (setf (status ps) (cons '? (cdr (car subgoal-traces-proofstates)))
	  ...
	  (t (format t "~%Bad rule: ~a~%" step);;treated as skip
					;(break)
	     (setf (status ps) (cons nil (make-trace 'empty 'np11));;start afresh
		   (strategy ps)
    ...
    (nprotecting-cong-state
     ...
     (let ((*rewrite-hash* (copy (rewrite-hash ps)))
       ...
       (loop for fmla in fmla-list
	     nconc
	     (multiple-value-bind (sig value)
		 (assert-if-i fmla (make-ctx 'not-implemented 'c126))
	       (cond ((tc-eq value *true*) nil)
		     ((eq (car sig) 'X) (list fmla))
		     (t (list value)))))))))
      ...
      (cond ((and *record-undone-proofstate*
	     ...
	     (let ((oldps (cadr *record-undone-proofstate*))
	       ...
	       (setf (justification ps)
		     (justification oldps)
		     (status ps)
		     (cons (status-flag oldps)
			   (make-trace 'not-implemented 'p11))
		 ...
		 (let ((response (pvs-y-or-n-p "Sure? ")))
		   (cond (response
			  ...
			  (setf (justification newps)
				...
				(status newps) (cons nil (make-trace 'empty '12))
...
(defmethod remove-dble-negation ((ex negation))
  (or (remove-dble-negation* (argument ex))
      (cons ex (make-trace 'proof-hole))))

(defmethod remove-dble-negation (ex)
  (cons ex (make-trace 'proof-hole)))

(defmethod remove-dble-negation* ((ex negation))
  (let ((result (remove-dble-negation (argument ex))))
    (cons (car result)
...
(defun clean-goal (goal)
  (let* ((trace (make-trace 'proof-hole))
	 (new-s-forms (loop for sf in (s-forms goal)
			    when (not (or (and (negation? (formula sf))
					       (tc-eq (args1 (formula sf))
						      *true*))
					  (tc-eq (formula sf) *false*)))
			    collect
			    (let ((result (remove-dble-negation (formula sf))))
			      (setq trace (apply-trace (cdr result) (list trace)))
			      (lcopy sf 'formula (car result))))))
    (loop for sf in (s-forms goal)
	  when (or (and (negation? (formula sf))
			(tc-eq (args1 (formula sf))
			       *true*))
		   (tc-eq (formula sf) *false*))
	  do
	  (setq trace (make-trace 'weak-rule
				  (list (formula sf))
				  (list trace))))
    (if (equal (s-forms goal) new-s-forms)
	(cons goal (make-trace 'proof-hole))
      (cons (lcopy goal 's-forms new-s-forms)
	      trace))))
	...
	(if (and (typep disjunct 'application)
	    ...
	    (cond  
	      ...
	      ((implication? disjunct)
	       (or+ (list (car (negate-with-proofs
	      ...
	      ((negation? disjunct)
		   ...
		   (let ((args2 (arguments (car (arguments disjunct)))))
			...
			(or+ (mapcar #'(lambda (x)(car (negate-with-proofs x)))
				     args2)))
		       ((negation? (args1 disjunct))
			(or+ (car args2)))
		       ((iff? (args1 disjunct))
			(or+ (list (car (negate-with-proofs (make-implication (car args2)
							     (cadr args2))))
				   (car (negate-with-proofs (make-implication
					    (cadr args2)
					    (car args2)))))))
		       (t (list disjunct))))
...
(defmethod and++ (form depth)
  ...
  (cons (list form) (make-trace 'proof-hole)))

(defmethod and++ ((form conjunction) depth)
  (let* ((result1 (and++ (args1 form) (when depth (1- depth))))
	 (result2 (and++ (args2 form) (when depth (1- depth)))))
    (cons (nconc (car result1) (car result2))
	  (make-trace 'and-rule
		      (list (args1 form) (args2 form))
		      (list (cdr result1) (cdr result2))))))

(defmethod and++ ((form iff) depth)
  (declare (ignore depth))
  (let* ((imp-res1 (make!-implication-with-proofs (args1 form) (args2 form)))
	 (imp-res2 (make!-implication-with-proofs (args2 form) (args1 form))))
    (cons (list (car imp-res1) (car imp-res2))
	  (make-trace 'iff-rule
		      (list (args1 form) (args2 form))
		      (list (car (cdr imp-res1))
			    (car (cdr imp-res2)))))))

(defmethod and++ ((form cases-expr) depth)
  (cons (car (and++ (translate-cases-to-if form) (when depth (1- depth))))
	(make-trace 'not-implemented 'p44)))

(defmethod and++ ((form branch) depth)
  (declare (ignore depth))
  (let* ((implication1 (make!-implication-with-proofs (condition form) (then-part form)))
	 (negation2 (negate!-with-proofs (condition form)))
	 (implication2 (make!-implication-with-proofs (car negation2) (else-part form))))
    (cons (list (car implication1) (car implication2))
	  (make-trace
	   'if-rule
	   (list (condition form) (then-part form) (else-part form))
	   (list
	    (car (cdr implication1))
	    (make-trace
	     'deep-inference-rule
	     (list (make-ctx 'application
			     (list (make-ctx 'expr (operator
						    (compute-implication (car negation2)
									 (else-part form))))
				   (make-ctx 'tuple
					     (list
					      (make-ctx 'expr-hole *boolean*)
					      (make-ctx 'expr (else-part form))))))
		   (car negation2)
		   (compute-negation (condition form)))
	     (list (car (cdr implication2))
		   (apply-trace (car (cdr negation2))
				(list (make-trace 'axiom-rule
						  (list (car negation2)))))
		   (apply-trace (cdr (cdr negation2))
				(list (make-trace 'axiom-rule
						  (list (compute-negation (condition form)))))))))))))

(defmethod and++ ((form negation) depth)
  (let ((result (and+-negation (argument form) depth)))
    (if (car result)
	result
      (cons (list form) (make-trace 'proof-hole)))))

(defmethod and+-negation ((form implication) depth)
  (let* ((result0 (negate!-with-proofs (args2 form)))
	 (result1 (and++ (car result0) (when depth (1- depth))))
	 (result2 (and++ (args1 form) (when depth (1- depth)))))
    (cons (nconc (car result1) (car result2))
	  (make-trace 'not-implication-rule
		      (list (args1 form) (args2 form))
		      (list (apply-trace (car (cdr result0))
					 (list (cdr result1)))
			    (cdr result2))))))

(defmethod and+-negation ((form disjunction) depth)
  (let ((result1 (and++ (negate! (args1 form)) (when depth (1- depth))))
	(result2 (and++ (negate! (args2 form)) (when depth (1- depth)))))
  (cons (nconc (car result1) (car result2))
...
(defmethod and+-negation ((form negation) depth)
  (let ((result (and++ (args1 form) (when depth (1- depth)))))
    (cons (car result)
	  (make-trace 'not-not-rule
	  	      (list (args1 form))
	  	      (list (cdr result))))))
...
(defmethod and+-negation ((form cases-expr) depth)
  (cons (car (and++ (negate! (translate-cases-to-if form)) (when depth (1- depth))))
	(make-trace 'not-implemented 'p49)))

(defmethod and+-negation ((form branch) depth)
  (declare (ignore depth))
  (let* ((neg-res0 (make!-negation-with-proofs (condition form)))
	 (conj-res1 (make!-conjunction-with-proofs (condition form) (then-part form)))
	 (conj-res2 (make!-conjunction-with-proofs (car neg-res0) (else-part form)))
	 (neg-res1 (make!-negation-with-proofs (car conj-res1)))
	 (neg-res2 (make!-negation-with-proofs (car conj-res2))))
    (cons (list (car neg-res1) (car neg-res2))
	  (make-trace
	   'not-if-rule
	   (list (condition form) (then-part form) (else-part form))
	   (list (make-trace
		  'deep-inference2-rule
		  (list (make-ctx 'negation (list (make-ctx 'expr-hole *boolean*)))
			(car conj-res1)
			(compute-conjunction (condition form) (then-part form)))
		  (list (car (cdr neg-res1))
			(car (cdr conj-res1)) (cdr (cdr conj-res1))))
		 (make-trace
		  'deep-inference2-rule
		  (list (make-ctx
			 'negation
			 (list (make-ctx
				'application
				(list (make-ctx 'expr
						(operator (compute-conjunction (car neg-res0)
									       (else-part form))))
				      (make-ctx 'tuple
		  ...
		  (list (make-trace
			 ...
			 (list (make-ctx 'negation (list (make-ctx 'expr-hole *boolean*)))
			       (car conj-res2)
			       (compute-conjunction (car neg-res0) (else-part form)))
			 (list (car (cdr neg-res2))
			       (car (cdr conj-res2)) (cdr (cdr conj-res2))))
			(car (cdr neg-res0)) (cdr (cdr neg-res0)))))))))

(defmethod and+-negation (form depth)
  (declare (ignore form depth))
  (cons nil
	(make-trace 'not-implemented 'p51)))
...
(defun split-step (sformnum ps depth labels)
  (let* ((goalsequent (current-goal ps))
    ...
    (if (or (null selected-sform)
	    ...
	    (eql depth 0))
	(values (cons 'X (make-trace 'empty 'np18))
		nil
		nil)
	(let* ((sel-sform (car selected-sform))
	       (new-sforms (delete-seq (s-forms goalsequent) (list sformnum)))
	       (result (and+ (formula sel-sform) depth))
	       (conjuncts (car result))
	       (conjunct-sforms
		(mapcar #'(lambda (x)
			    (copy sel-sform
				  'formula x))
