...
;;simplifier:[sform -> sform/list of sforms]
(defun sform-reduce-with-proofs (sformlist simplifier sformnums pos neg)
  (if (null sformlist) (values (cons 'X (make-trace 'proof-hole)) nil)
    (let* ((x (car sformlist))
	   (sign (not (negation? (formula x)))))
      (multiple-value-bind
       (signal result)
       (if (in-sformnums? x pos neg sformnums)
	   (funcall simplifier x)
	 (values (cons 'X (make-trace 'proof-hole)) x));;no change
       (let ((result (if (listp result) result (list result))))
	 (cond ;; Nine cases in general '!, '? or 'X for (car signal) and (car rsignal)
	  ((eq (car signal) '!) ;; Three cases, (car signal) being '!
	   (cons '! (cdr signal)))
	  (t (multiple-value-bind ;; Six cases (car signal) being 'X or '?
	      (rsignal rresult)
	      (sform-reduce-with-proofs (cdr sformlist) simplifier sformnums
					(if sign (1+ pos) pos)
					(if sign neg (1- neg)))
	      (let ((presignal
		     (if (eq (car rsignal) '!)
			 (cons '! (cdr rsignal)) ;; Covers ('?,'!) and ('X,'!)
		       (cons (if (eq (car signal) 'X);;compute result signal
				 (car rsignal)    ;; Covers ('X, '?) and ('X, 'X)
			       (car signal))  ;; Covers ('?, '?) and ('?, 'X)
			     (apply-trace (cdr signal) (list (cdr rsignal)))))))
		(let* ((false-or-not-true?
			#'(lambda (x)
			    (with-slots (formula)
					x
					(or
					 ...
					 (and (negation?
					      ...
					      (tc-eq (args1
						      formula)
						     *true*))))))
		       (trace-cleaned-result
			(reduce #'(lambda (current sform)
				    (if (funcall false-or-not-true? sform)
					(cons (apply-trace
					       (car current)
					      ...
					      (cdr current))
				      (cons (car current)
					    (cons sform (cdr current)))))
				(cons (cons (cdr presignal) nil) result))))
		  (values (cons (car presignal)
				(car trace-cleaned-result))
			  (nconc (reverse (cdr trace-cleaned-result))
				 rresult))))))))))))
...
(defun sequent-reduce-with-proofs (sequent simplifier sformnums)
  (multiple-value-bind
   (signal result)
   (sform-reduce-with-proofs (s-forms sequent) simplifier
		 (cleanup-fnums sformnums) 1 -1)
   (cond ((eq (car signal) '!) (values signal nil))
	 ((eq (car signal) 'X) (values signal sequent))
	  ((eq (car signal) '?)(values signal (lcopy sequent
						     's-forms
						     result))))))
...
(defmethod simplify-disjunct ((formula application) depth)
  (if (or (and (integerp depth)
      ...
      (cons (list formula) (make-trace 'proof-hole))
...
(defmethod simplify-disjunct* ((formula disjunction) depth)
  (if (equal (length (arguments formula)) 2)
      (let* ((result1 (simplify-disjunct (args1 formula) depth))
	     (result2 (simplify-disjunct (args2 formula) depth)))
	(cons (nconc (car result1)
		     (car result2))
	      (make-trace 'or-rule
			  (list (args1 formula) (args2 formula))
			  (list
			   (apply-trace (cdr result1)
					(list (cdr result2)))))))
    (cons (loop for arg in (arguments formula)
		nconc (car (simplify-disjunct arg depth)))
	  (make-trace 'not-implemented 'p23))))

(defmethod simplify-disjunct* ((formula implication) depth)
  (let* ((result0 (negate-with-proofs (args1 formula)))
	 (neg-args1 (car result0))
	 (neg-trace (car (cdr result0)))
	 (result1 (simplify-disjunct neg-args1 depth))
	 (result2 (simplify-disjunct (args2 formula) depth)))
    (cons (nconc (car result1)
		 (car result2))
	  (make-trace 'implication-rule
		      (list (args1 formula) (args2 formula))
		      (list
		       (apply-trace neg-trace
				    (list (apply-trace (cdr result1)
						       (list (cdr result2))))))))))

(defmethod simplify-disjunct* ((formula disequation) depth)
  (declare (ignore depth))
  (let* ((neg-res (negate-with-proofs (make!-equation (args1 formula)
						      (args2 formula))))
	 (newform (car neg-res)))
    (cons (list newform)
	  (make-trace 'rewrite-rule
		      (list (make-ctx 'expr-hole *boolean*)
			    (compute-negation (compute-equation (args1 formula)
						      (args2 formula)))
			    formula)
		      (list (car (cdr neg-res)))))))

(defmethod simplify-disjunct* ((formula negation) depth)
  (let ((result (simplify-disjunct-neg (args1 formula) depth)))
    (if (car result)
	result
      (cons (list formula) (make-trace 'proof-hole)))))
...
(defmethod simplify-disjunct-neg ((formula application) depth)
  (if (typep (operator formula) 'name-expr)
      (simplify-disjunct-neg* formula depth)
    (cons nil (make-trace 'not-implemented 'p36))))

(defmethod simplify-disjunct-neg (formula depth)
  (declare (ignore formula depth))
  (cons nil (make-trace 'not-implemented 'p37)))

(defmethod simplify-disjunct-neg* ((formula conjunction) depth)
  (if (equal (length (arguments formula)) 2)
      (let* ((nresult1 (negate-with-proofs (args1 formula)))
	     (nresult2 (negate-with-proofs (args2 formula)))
	     (result1 (simplify-disjunct (car nresult1) depth))
	     (result2 (simplify-disjunct (car nresult2) depth)))
	(cons (nconc (car result1)
		     (car result2))
			  ...
			  (list (apply-trace
				 ...
				 (list (apply-trace
					...
					(list (apply-trace
					       ...
					       (list (cdr result2)))))))))))
    (cons (loop for arg in (arguments formula)
		nconc (car (simplify-disjunct (car (negate-with-proofs arg)) depth)))
	  (make-trace 'not-implemented 'p25))))

(defmethod simplify-disjunct-neg* ((formula disequation) depth)
  (declare (ignore depth))
  (cons (list (make!-equation (args1 formula)
			      (args2 formula)))
	(make-trace 'rewrite-rule
		    (list (make-ctx 'negation (list (make-ctx 'expr-hole *boolean*)))
			  (compute-negation (compute-equation (args1 formula)
							      (args2 formula)))
			  formula)
		    (list (make-trace 'not-not-rule
				      (list (compute-equation (args1 formula)
							      (args2 formula)))
				      (list (make-trace 'proof-hole)))))))

(defmethod simplify-disjunct-neg* ((formula negation) depth)
  (let ((result (simplify-disjunct (args1 formula) depth)))
    (cons (car result)
...
(defmethod simplify-disjunct-neg* ((formula iff) depth)
  (let* ((imp-res1 (make!-implication-with-proofs (args1 formula) (args2 formula)))
	 (imp-res2 (make!-implication-with-proofs (args2 formula) (args1 formula)))
	 (neg-res1 (negate-with-proofs (car imp-res1)))
	 (neg-res2 (negate-with-proofs (car imp-res2)))
	 (res1 (simplify-disjunct (car neg-res1) depth))
	 (res2 (simplify-disjunct (car neg-res2) depth)))
    (cons (nconc (car res1) (car res2))
	  (make-trace
	   'not-iff-rule
	   (list (args1 formula) (args2 formula))
	   (list
	    (make-trace
	     ...
	     (list
	      (apply-trace
	       ...
	       (list (make-trace
		      'axiom-rule
		      (list (compute-implication (args1 formula) (args2 formula))))))
	      (make-trace
	       'weak-rule
	       (list (compute-negation (compute-implication (args1 formula) (args2 formula))))
	       (list
		(make-trace
		 'cut-rule
		 (list (car imp-res2))
		 (list
		  (apply-trace
		   ...
		   (list (make-trace
			  'axiom-rule
			  (list (compute-implication (args2 formula) (args1 formula))))))
		  (make-trace
		   'weak-rule
		   (list (compute-negation (compute-implication (args2 formula) (args1 formula))))
		   (list
		    (apply-trace
		     (car (cdr neg-res1))
		     (list
...
(defun lift-if-expr (expr) ;;NSH(4.5.95) utility for if-lifting
  ...
  (let* ((conds (collect-conds expr))
	 ...
	 (if-expr (car (make-top-if-expr expr conds))))
...
;; proof of trueconds, not falseconds |- output = expr
	  ...
	  (if (consp (car conds))   ;;negated condition
		  ...
		  (let ((result
			 (make!-if-expr (condition expr)
					(then-part expr)
					(car (make-top-level-if-expr*
					      (else-part expr)
					      (cdr conds)
					      trueconds
					      (cons (condition expr) falseconds))))))
		    (cons result (make-trace 'trusted-rule
					     (list 'ite1
						   nil
						   (list (compute-equation result expr))))))
		;;shouldn't happen
		(cons expr
		      (assert (and 'ite11 nil))))
...
;			       :test #'tc-eq)
	 (result (make-top-level-if-expr body conds))
	 (if-expr (car result))
	 (proof (cdr result)))
    (if (null conds)
	 ...
	 (if (truecond? cond trueconds falseconds)
	   ...
	   (if (falsecond? cond trueconds falseconds)
		...
		(make-trace 'trusted-rule
			    (list 'truefalsecond
				  nil
				  (list (compute-equation carresult cond)))))))
...
(defun make-top-if-expr (expr conds &optional trueconds falseconds)
  (cond ((null conds)
	...
	(t (let* ((resultthen
		  ...
		  (thenproof1
		   (cdr resultthen))
		  (newthen (car (truefalsecond-reduce
		  ...
		  (elseproof1
		   (cdr resultelse))
		  (newelse (car (truefalsecond-reduce
			    newelse1 trueconds falseconds))))
	     (if (tc-eq newthen newelse)
		 (cons newthen
		       (assert (and 'top-ite2 nil)))
		 (if (and (eq newthen *true*)
			  (eq newelse *false*))
		     (cons (car conds)
			   (assert (and 'top-ite3 nil)))
		   (let ((result (make!-if-expr (car conds) newthen newelse))
			 (original (compute-if (car conds) newthen newelse)))
		     (assert (compute-form-eq result original))
		     (assert (compute-form-eq newthen1 newthen));; FG to be handled
		     (assert (compute-form-eq newelse1 newelse));; FG to be handled
		     (cons
		      result
		      (make-trace
		       'trans-rule
		       (list result (compute-if (car conds) expr expr) expr)
		       (list
			(make-trace
			 'trans-rule
			 (list result (compute-if (car conds) newthen expr)
			       (compute-if (car conds) expr expr))
			 (list
			  (make-trace
			   'if-else-congruence-rule
			...
			(make-trace
			 ...
			 (list 'ifthenelse
			       nil
			       (list (compute-equation (compute-if (car conds) expr expr)
			 			 expr))))))))))))))
  ...
  (let ((top-hash-value (gethash
	...
	(cons expr
	      (make-trace 'trusted-rule
			     (list 'simplify-ifs1
				   nil
				   (list (compute-equation expr expr)))))
	(let* ((result (call-next-method))
	       (result
		(if (and (type (car result))
			 (tc-eq (find-supertype (type (car result)))
				*boolean*))
		    (let ((truefalseresult (truefalsecond-reduce (car result)
								 trueconds
								 falseconds)))
		      (cons (car truefalseresult)
			    (make-trace
			     'trans-rule
			     (list (car truefalseresult)
				   (car result)
				   expr)
			     (list (cdr truefalseresult)
				   (cdr result)))))
		    result)))
	  (cond ((eq (car result) expr)
...
(defmethod simplify-ifs ((expr branch) trueconds falseconds)
  (let* ((condition-result (simplify-ifs (condition expr) trueconds falseconds))
	 (simple-condition (car condition-result)))
    ;;(break "simplify if")
    (if (eq simple-condition *true*)
	;;(truecond? simple-condition trueconds falseconds)
	(let ((result (simplify-ifs (then-part expr) trueconds falseconds)))
	  (cons (car result)
		(make-trace
		 'trans-rule
		 (list (car result)
		       (compute-if *true* (then-part expr) (else-part expr) (type expr))
		       expr)
		 (list
		  (make-trace
		   'trans-rule
		   ...
		   (list (cdr result)
			 (make-trace
			  ...
			  (list 'simplify-ifs
				nil
				(list (compute-equation
				       (then-part expr)
				       (compute-if *true* (then-part expr)
						   (else-part expr) (type expr))))))))
		    (make-trace 'congruence-rule
		    		(list
		    		 (make-ctx
		    		  'application
		    		  (list (make-ctx 'expr
	  ...
	  (let ((result (simplify-ifs (else-part expr) trueconds falseconds)))
	    (cons (car result)
		  (make-trace
		   'trans-rule
		   (list (car result)
			 (compute-if *false* (then-part expr) (else-part expr) (type expr))
			 expr)
		   (list
		    (make-trace
		     'trans-rule
		     ...
		     (list (cdr result)
			   (make-trace
			    ...
			    (list 'simplify-ifs
				  ...
				  (list (compute-equation
					 (else-part expr)
					 (compute-if *false* (then-part expr)
						     (else-part expr) (type expr))))))))
	...
	(cons carresult
	      (make-trace 'trusted-rule
			  (list 'simplify-ifs4
				nil
				(list (compute-equation carresult expr))))))))))
	  
(defmethod simplify-ifs ((expr binding-expr) trueconds falseconds)
  (let ((carresult
  (with-slots (expression) expr
    (lcopy expr
      'expression (car (simplify-ifs expression trueconds falseconds)))))
	) (cons carresult
		(make-trace 'trusted-rule
			    (list 'simplify-ifs5
				  nil
...
(defmethod simplify-ifs ((expr projection-application) trueconds falseconds)
  ...
  (with-slots (index argument) expr
    (let ((arg (car (simplify-ifs argument trueconds falseconds))))
      (if (eq arg argument)
	  expr
		 ...
		 (if (and (branch? translation)
			  (or (truecond? (condition translation)
			      ...
			      (falsecond? (condition translation)
					  trueconds falseconds)))
		     (car (simplify-ifs translation trueconds falseconds))
		   (let* ((op (car (simplify-ifs operator trueconds falseconds)))
			  (arg (car (simplify-ifs argument trueconds falseconds)))
		     ...
		     (lcopy expr
		       ...
		       (make-trace 'trusted-rule
				   (list 'simplify-ifs111
					 nil
					 (list (compute-equation carresult expr))))))
      (let* ((op (car (simplify-ifs operator trueconds falseconds)))
	     (argresult (simplify-ifs argument trueconds falseconds))
	     (arg (car argresult))
	     (argproof (cdr argresult)))
	(if (and (eq op operator)
	  ...
	  (let* ((stype (find-supertype (type op)))
		 (interm (copy expr
			       ...
			       'type (if (typep (domain stype) 'dep-binding)
					 (substit (range stype)
						  (acons (domain stype) argument nil))
				       (range stype))))
		 (carresult (copy expr
				  'operator op
				  'argument arg
				  'type (if (typep (domain stype) 'dep-binding)
					    (substit (range stype)
						     (acons (domain stype) arg nil))
					  (range stype)))))
	    (assert (compute-form-eq (compute-application op argument) interm));;FG 
	    (assert (compute-form-eq (compute-application op arg) carresult));;FG 
	    (cons carresult
		  (make-trace
			 'trans-rule
			 (list carresult interm expr)
			 (list
			  (make-trace 'congruence-rule
				      (list (make-ctx 'application
						      (list (make-ctx 'expr op)
							    (make-ctx 'expr-hole (type argument))))
					    arg
					    argument)
				      (list argproof))
			  (make-trace 'trusted-rule
			      (list 'simplify-ifs1112
				    nil
				    (list (compute-equation interm expr))))))))))
      )))
...
(defmethod simplify-ifs ((expr record-expr) trueconds falseconds)
  ...
  (lcopy expr
	'assignments (car (simplify-ifs assignments
				   trueconds falseconds)))))
	) (cons carresult
		(make-trace 'trusted-rule
			    (list 'simplify-ifs12
...
(defmethod simplify-ifs ((expr tuple-expr) trueconds falseconds)
  (with-slots (exprs) expr
    (let* ((sexprsresults (simplify-ifs exprs trueconds falseconds))
	   (sexprs (car sexprsresults))
	   (sexprsproofs (cdr sexprsresults)))
      (if (eq exprs sexprs)
	  (cons expr
		(make-trace 'refl-rule (list expr)))
	(let* ((splitexprs (splits-list exprs))
	       (splitsexprs (splits-list sexprs))	       
	       (splits (mapcar #'cons splitexprs splitsexprs))
	       (splits-proofs (mapcar #'cons splits sexprsproofs))
	       (carresult (copy expr
				'exprs sexprs
				'type (mk-tupletype (mapcar #'type sexprs)))))
	  (cons carresult
		(reduce
		 #'(lambda (current splits-proofs);;  sexprs1,expr,exprs2 = exprs
		     (let* ((splits (car splits-proofs))
			    (proof (cdr splits-proofs))
			    ;; (exprs1 (car (car splits)))
			    (cexpr (cadr (car splits)))
			    (exprs2 (caddr (car splits)))
			    (sexprs1 (car (cdr splits)))
			    (csexpr (cadr (cdr splits)))
			    ;; (sexprs2 (caddr (cdr splits)))
			    )
		       (make-trace 'trans-rule
				   (list (compute-tuple (append sexprs1 (list csexpr) exprs2))
					 (compute-tuple (append sexprs1 (list cexpr) exprs2))
					 expr)
				   (list (make-trace
					  'congruence-rule
					  (list (make-ctx 'tuple
							  (append (mapcar
								   #'(lambda (ex) (make-ctx 'expr ex))
								   sexprs1)
								  (list (make-ctx 'expr-hole
										  (type cexpr)))
								  (mapcar
					  ...
					  (list proof))
					 current))))
		 (cons (make-trace 'refl-rule (list expr))
		       splits-proofs))
		;; (cons carresult
		;;       (make-trace 'trusted-rule
		;; 		  (list 'simplify-ifs13
...
(defmethod simplify-ifs ((expr update-expr) trueconds falseconds)
  ...
  (lcopy expr
	'expression (car (simplify-ifs expression
				  trueconds falseconds))
	'assignments (car (simplify-ifs assignments
				   trueconds falseconds)))))
	) (cons carresult
...
(defmethod simplify-ifs ((expr assignment) trueconds falseconds)
  ...
  (lcopy expr
    'arguments (car (simplify-ifs arguments trueconds falseconds))
    'expression (car (simplify-ifs expression
				  trueconds falseconds)))))
	) (cons carresult
		(make-trace 'trusted-rule
...
(defmethod simplify-ifs ((expr cases-expr) trueconds falseconds)
  ...
  (let* ((expression (car (simplify-ifs expression trueconds falseconds)))
	...
	(car (simplify-ifs else-part trueconds falseconds))
	(if (and (singleton? non-false-selections) (null else-part))
	    ...
	    (car (simplify-ifs  (subst-accessors-in-selection
			    expression (car non-false-selections))
			   trueconds falseconds))
	    (if true-selection
		(car (simplify-ifs true-selection trueconds falseconds))
		(lcopy expr
		  'expression expression
		  'selections (car (simplify-ifs selections
					    trueconds falseconds))
		  'else-part (car (simplify-ifs else-part
					   trueconds falseconds)))))))))
	) (cons carresult
		(make-trace 'trusted-rule
			    (list 'simplify-ifs16
  ...
  (protect-types-hash
   ...
   (let* ((name-expr (pc-parse name 'name))
     ...
     (cond ((not (listp substs))
	    ...
	    (values (cons 'X (make-trace 'not-implemented 'p323)) nil))
	   ((not (null badnames))
	    (error-format-if
	     "~%The form of a substitution is: (<var1> <term1>...<varn> <termn>).
The following are not possible variables: ~{~a,~}" badnames)
	    (values (cons 'X (make-trace 'not-implemented 'p345)) nil))
	   ((oddp (length substs))
	    (error-format-if
	     "~%The form of a substitution is: (<varn> <termn>...<varn> <termn>).")
	    (values (cons 'X (make-trace 'not-implemented 'p346)) nil))
	   (t (let* ((all-possibilities
		...
		(let ((subfreevars (loop for (nil . y) in newalist
		  ...
		  (cond ((null resolutions)
			 ...
			 (values (cons 'X (make-trace 'not-implemented 'p347)) nil))
			((null form)
...
or supply more substitutions."
			 ...
			 (values (cons 'X (make-trace 'not-implemented 'p348)) nil))
			(subfreevars
			 (error-format-if "~%Irrelevant free variables ~a in substitution."
					  subfreevars)
			 (values (cons 'X (make-trace 'not-implemented 'p349)) nil))
			(t
			 (let* 
				...
				subvars :test #'same-id))
				 ...
				 (substit intermediate-form bindalist)))
			      (result (negate-with-proofs subform))
			   ...
			   (values
			    (cons '?
				  (make-trace
				   ...
				   (list
				    (make-trace 'trusted-rule
						(if (string= (format nil "~a" name)
							     "naturalnumbers.nat_induction")
						    (list 'nat_induction
							nil (list subform))
						  (list 'lemma-proofrule
							nil (list subform))))
				    (car (cdr result)))))
			    (list newsequent)
					;NSH(4.9.99)
...
(defun typepred-step (exprs all? implicit? ps);;(2004-09-14) added implicit?
  (let ((preds (if implicit?
    ...
    (if (null preds)
	(values (cons 'X (make-trace 'not-implemented 'p14)) nil nil)
	(let* ((new-sforms
		(mapcar #'(lambda (fmla)
	  ...
	  (values (cons '?
			(make-trace 'typepred-rule
				    (list (mapcar #'(lambda (fmla) (car (negate-with-proofs fmla)))
						  preds))
				    (list (make-trace 'proof-hole))))
		   (list
  ...
  (let ((fmla (formula sform)))
    (cond ((and (negation? fmla)
	  ...
	  (values '? (lcopy sform
			   'formula
			   (typecheck
			    (car (negate-with-proofs (mk-iff (args1 (args1 fmla))
					      (args2 (args1 fmla)))))
			    :expected *boolean*
			    :context *current-context*))))
	  ((and (equation? fmla)
...
(defun case-rule-fun (fmlas)
  #'(lambda (ps)
	...
	(cond ((null tc-fmlas)
	       ...
	       (values (cons 'X (make-trace 'empty 'np29)) nil nil))
	      ((not (null freevars))
	       (error-format-if
		"~%Irrelevant free variables ~{~a, ~} occur in formulas."
		freevars)
	       (values (cons 'X (make-trace 'empty 'np30)) nil nil))
	      (t 
	       (multiple-value-bind
		(trace subgoals dependent-decls)
		(make-cases (current-goal ps) tc-fmlas (make-trace 'proof-hole) nil)
		(values (cons '? trace) subgoals
			(list 'dependent-decls dependent-decls))))))))

(defun make-cases (goal fmlas trace &optional references accum)
  (cond ((null fmlas) (values trace (cons goal accum)
			      references))
	(t (let* ((neg-res (negate-with-proofs (car fmlas)))
		  (neg-fmla (car neg-res))
...
;			     references
	       (make-cases neg-goal (cdr fmlas)
			   (apply-trace
			    trace
			    (cons (make-trace 'cut-rule
			    		      (list (compute-negation (car fmlas)))
			    		      (list (car (cdr neg-res))
			    			    (make-trace 'not-not-rule
			    					(list (car fmlas))
			    					(list (make-trace 'proof-hole)))))
			    	  (mapcar #'(lambda (x) (declare (ignore x))
			    		      (make-trace 'proof-hole)) accum)))
			   references
			   (cons pos-goal accum)))))))
...
occur in the given type expression: ~a." freevars)
	       (values (cons 'X (make-trace 'empty 'np26)) nil))
...
which should be fully instantiated. Please supply actual parameters.")
	       (values (cons 'X (make-trace 'empty 'np27)) nil))
	      (t
	       (multiple-value-bind
		(signal subgoals updates)
		(extensionality-step tc-expr tc-expr ps)
		(assert (null (cdr subgoals)));;FG
		(values (cons signal
			      (make-trace 'trusted-rule
					  (list 'extensionality-proofrule
						(list (mapcar #'formula (s-forms (car subgoals))))
	   ...
	   (let* ((references nil))
	     ...
	     (values '?
		     (list (copy (current-goal ps)
			     ...
			     (cons (make-instance 's-formula
				     :formula
				     (car (negate-with-proofs new-fmla)))
				   (s-forms (current-goal ps)))))
		     (list 'dependent-decls
			   references))))
	     ...
	     (let* ((name (typecheck (pc-parse name 'expr)
	       ...
	       (values '?
			...
			(cons (copy (current-goal ps)
				...
				(cons (make-instance 's-formula
					:formula
					(car (negate-with-proofs formula)))
				      (s-forms (current-goal ps))))
