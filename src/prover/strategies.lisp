  ...
  (let ((fnum (find-sform (s-forms (current-goal *ps*)) fnum))
	...
	(let ((fmla (formula (car sforms)))
	      (form (if (negation? fmla)
			(args1 fmla)
			(if (exists-expr? fmla)
			    (car (negate-with-proofs (change-class (copy fmla)
						  'forall-expr)))
			    fmla)))
		...
		(let ((res (car resolutions))
		  ...
		  (if info
		      (let ((conc (car info))
			    ...
			    (formlist (append *-*
					      (mapcar #'(lambda (expr) (car (negate-with-proofs expr))) *+*)))
  ...
  (if (consp fnums) ;;fnums has to be a list from chain-antecedent
      (let ((fnum (car fnums))
	    (fmla1 (formula (car (select-seq (s-forms (current-goal *ps*))
				    fnum))))
	    (nfmla1 (car (negate-with-proofs fmla1)))
...
(defun forward-match*-rec (conc antec-fmlas formlist all-formlist subst)
  (cond ((null antec-fmlas)
	 (cond ((subsetp (car (and+ (substit conc subst))) all-formlist
			:test #'tc-eq)
...
(defun conjuncts (fmla)
  (cond ((negation? fmla)
	 (mapcar #'(lambda (expr) (car (negate-with-proofs expr)))  (disjuncts (args1 fmla))))
	((conjunction? fmla)
...
(defun disjuncts (fmla)
  (cond ((negation? fmla)
	 (mapcar #'(lambda (expr) (car (negate-with-proofs expr))) (conjuncts (args1 fmla))))
	((disjunction? fmla)
	 (nconc (disjuncts (args1 fmla))(disjuncts (args2 fmla))))
	((implication? fmla)
	 (nconc (mapcar #'(lambda (expr) (car (negate-with-proofs expr))) (conjuncts (args1 fmla)))
  ...
  (let ((op (pc-parse op 'name))
	...
	(new-ineq-conjunction
	 (when ineq-conjunction
	   (let* ((ineq-conjuncts (car (and+ ineq-conjunction)))
		  (new-conjuncts
