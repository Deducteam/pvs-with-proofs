...
(defmethod set-type-actuals ((modinst modname) &optional theory)
  (unless theory (setq theory (get-theory modinst)))
  (assert theory)
  (when (actuals modinst)
    (let ((fmls (formals-sans-usings theory))
	  (acts (actuals modinst)))
      (set-type-actuals* acts fmls)
      (assert (every #'typed? acts))))
  (when (dactuals modinst)
    (let ((decl (declaration modinst)))
      (assert decl)
      (let ((fmls (decl-formals decl))
	    (acts (dactuals modinst)))
	(set-type-actuals* acts fmls)
	(assert (every #'typed? acts)))))
  (when (or (actuals modinst)
	    (dactuals modinst))
...
(defmethod set-type-actuals ((expr name) &optional theory)
  ...
  (let* ((modinst (module-instance expr))
    ...
    (when (and (not *dont-worry-about-full-instantiations*)
	       (or (some #'null (actuals modinst))
		   (some #'null (dactuals modinst))))
      (type-error expr
    ...
    (let ((*generate-tccs* (cond ((eq *generate-tccs* 'none) 'none)
				 ...
				 (t *generate-tccs*)))
	  (fmls (unless (eq thry (current-theory))
		  (formals-sans-usings thry))))
      (set-type-actuals* (actuals modinst) fmls)
      (when (decl-formals (declaration expr))
	(set-type-actuals* (dactuals modinst)
			   (decl-formals (declaration expr)))))
    (when (typep modinst 'modname-no-tccs)
      (change-class modinst 'modname))
    #+pvsdebug (fully-typed? (actuals modinst))
    #+pvsdebug (fully-typed? (dactuals modinst))
	...
	(mapc #'(lambda (ea ma)
		  (cond ((type-value ma)
			...
			(t (setf (type-value ea) nil)
			   (set-type* (expr ea) (type (expr ma))))))
	      (dactuals expr) (dactuals modinst))))
    (let ((nmodinst (simplify-modinst modinst)))
      (unless (eq *generate-tccs* 'none)
...
(defun instantiate-operator-type (optype op args expected)
  (let* ((frees (free-params optype))
	...
	(cond ((null th) (setq th (module (car bd))))
	      ((not (eq (module (car bd)) th)) (break "Check mixed optype params")))
...
(defun bindings-to-modinsts (bindings modinsts theories)
  (if (null bindings)
      ...
      (let* ((theory (module (caar bindings)))
	     ...
	     (decl (when rem (associated-decl (caar rem))))
...
(defun maybe-instantiate-from-decl-formals (ex)
  (let* ((dfmls (decl-formals (current-declaration)))
	 (fparams (when dfmls (free-params ex)))
	 (adecl (when fparams (associated-decl (car fparams)))))
    (when (and fparams (length= fparams dfmls)
	       (every #'(lambda (fp)
			  (and (decl-formal? fp)
